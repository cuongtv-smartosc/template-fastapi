...
def vehicles_list_base_filter(params, group_by, query):
    ...
    if 'work_shift' in params:
        query = query.inner_join(WorkShift).on(Vehicle.name == WorkShift.vehicle_number)
        work_shifts = params["work_shift"]
        work_shift_names = []
        for item in work_shifts:
            work_shift_name = WorkShift.query(WorkShift.name).filter(WorkShift.work_shift == item.get("shift"),
                                                                       WorkShift.work_shift_from == item.get(
                                                                           "ws_from"),
                                                                       WorkShift.work_shift_to == item.get(
                                                                           "ws_to")).all()
