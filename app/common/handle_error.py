...
class NotFoundException(APIException):
    def __init__(self, message=ErrorMessages.not_found, extra=None):
        super().__init__(http_status=status.HTTP_404_NOT_FOUND, message=message, extra=extra)
...
class BadRequestException(APIException):
    ...
    def __init__(self, message=ErrorMessages.bad_request, extra=None):
        super().__init__(http_status=status.HTTP_400_BAD_REQUEST, message=message, extra=extra)
...
class MethodNotAllowed(APIException):
    def __int__(self, message=ErrorMessages.method_not_allow, extra=None):
        super().__init__(http_status=status.HTTP_405_METHOD_NOT_ALLOWED, message=message, extra=extra)
